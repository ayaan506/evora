...
ADMIN_EMAILS = [
    "admin@evora.com",
    "owner@evora.com",
    "ayaanpathan1617@gmail.com",
]

# SMTP & Brevo API Settings
...
def init_db():
  ...
  if "payment_id" not in cols:
    cursor.execute(
        "ALTER TABLE bookings ADD COLUMN payment_id TEXT DEFAULT 'N/A'"
    )
...
# ==========================================================================
# TRANSACTIONAL EMAIL TEMPLATES & SENDER (Brevo HTTP API / Fallback)
# ==========================================================================
...
def get_welcome_email_template(fullname, email):
  return f"""
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .card {{ max-width: 540px; margin: 0 auto; background-color: #111827; border-radius: 12px; border: 1px solid #1f2937; overflow: hidden; }}
            ...
            .content {{ padding: 28px; line-height: 1.6; color: #cbd5e1; font-size: 15px; }}
            .btn-wrap {{ text-align: center; margin: 28px 0; }}
            .btn {{ background: #4f46e5; color: #ffffff !important; padding: 12px 28px; text-decoration: none; border-radius: 8px; font-weight: 600; display: inline-block; }}
            .footer {{ border-top: 1px solid #1f2937; padding: 16px; text-align: center; font-size: 12px; color: #6b7280; }}
        </style>
    </head>
    <body>
        <div class="card">
            ...
            <div class="content">
                ...
                <p>Your account (<strong>{email}</strong>) has been successfully verified and activated.</p>
                <p>You can now browse live concerts, book exclusive passes, download verified QR passes, and manage event operations.</p>
                <div class="btn-wrap">
                    <a href="https://evora-0m5i.onrender.com" class="btn">Explore Events</a>
                </div>
            </div>
            <div class="footer">
...
def get_venue_approval_email_template(
    fullname, venue_name, booking_date, shift_type, total_price, booking_ref
):
  return f"""
    <div style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f8fafc; padding: 40px 15px; color: #0f172a; line-height: 1.6;">
        <div style="max-width: 580px; margin: 0 auto; background: #ffffff; border-radius: 20px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.04);">
            ...
            <div style="padding: 32px 28px;">
                ...
                <div style="text-align: center; margin: 28px 0;">
                    <a href="/dashboard" style="display: inline-block; background: linear-gradient(135deg, #059669, #10b981); color: #ffffff; text-decoration: none; font-weight: 700; font-size: 14px; padding: 14px 32px; border-radius: 10px; box-shadow: 0 4px 15px rgba(5, 150, 105, 0.3);">
...
def send_email_direct(to_email, subject, html_content):
  """Sends email via Brevo REST API over HTTPS (Port 443 - works on Render) with SMTP fallback"""
  api_key = os.getenv("BREVO_API_KEY", "")
  sender = os.getenv("SENDER_EMAIL", SENDER_EMAIL)

  # 1. Primary Method: Brevo HTTPS API
  if api_key:
    ...
    payload = {
        "sender": {"name": "Evora Live", "email": sender},
        "to": [{"email": to_email}],
        "subject": subject,
    ...
    try:
      ...
      if response.status_code in [200, 201, 202]:
        print(f"✅ [BREVO SENT] Successfully delivered to {to_email}")
        return True, "Success"
      else:
  ...
  # 2. Fallback: Direct SMTP (Local Dev)
  clean_sender = (sender or "").strip()
  clean_password = (os.getenv("SENDER_PASSWORD", SENDER_PASSWORD) or "").strip()

  if clean_sender and clean_password:
    try:
      ...
      print(f"✅ [SMTP SENT] Delivered to {to_email}")
      return True, "Success"
    except Exception as e:
      print(f"⚠️ [SMTP BLOCKED/FAILED]: {e}")

  print(
      f"ℹ️ [BYPASS] Email dispatch logged without crashing workflow for"
      f" {to_email}."
  )
  return True, "Success"
...
@app.route("/events")
def events():
  return render_template("concerts_hub.html")


@app.route("/event-detail")
def event_detail():
  return render_template("concert_detail.html")
...
def verify_and_register():
  try:
    ...
    cursor.execute(
        "SELECT otp, expires_at FROM otp_codes WHERE email = ? ORDER BY id DESC"
        " LIMIT 1",
        (email,),
    )
    rec = cursor.fetchone()
    ...
    try:
      ...
      print(f"⚠️ [WELCOME EMAIL LOG ERROR]: {email_err}")
...
def login():
  try:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT fullname, password, role FROM users WHERE email = ?", (email,)
    )
    ...
    is_valid = (
        check_password_hash(db_hash, password)
        if db_hash.startswith(("pbkdf2:", "scrypt:"))
        else (password == db_hash)
    )
    ...
    if is_valid:
      ...
      log_cur.execute(
          "INSERT INTO login_logs (user_email, fullname, login_time, ip_address,"
          " status) VALUES (?, ?, ?, ?, ?)",
          (
              email,
              db_name,
...
def get_event_inventory(event_id):
  ...
  return (
      jsonify(
          {"status": "success", "event_id": event_id, "inventory": inventory}
      ),
      200,
  )
...
def mobile_pay_screen(session_id):
  ...
  if not session_info:
    return (
        ...
        " margin-top:50px;'>QR Code Expired or Invalid</h2>"
    ), 404
...
def verify_payment_and_book():
  try:
    ...
    razorpay_payment_id = data.get(
        "razorpay_payment_id", f"PAY-EVR-{random.randint(1000,9999)}"
    )
    ...
    if existing_booking_id:
      ...
      generated_passes.append(
          {"ticket_ref": ref_id, "tier_name": title, "price": total_price}
      )
    ...
    # 2. Multi-Tier Concert Passes
      ...
      for tk, req_qty in tier_request_counts.items():
        cursor.execute(
            "SELECT available_seats, tier_name FROM event_inventory WHERE"
            " event_id = ? AND tier_key = ?",
            (event_id, tk),
        )
        ...
        if not inv_row or inv_row[0] < req_qty:
          ...
          return (
              jsonify({
                  "status": "sold_out",
                  "message": (
                      f"Sorry! {t_name} has just sold out or does not have"
      ...
      for index, item in enumerate(individual_items, start=1):
        ...
        cursor.execute(
            ...
            (
                ...
                "passes",
                f"{title} — Pass #{index}",
                f"Tier: {tier_name} • Pass {index} of {len(individual_items)} •"
                f" Venue: {details}",
                booking_date,
                tier_price,
                ticket_ref,
    ...
    tickets_html_list = "".join([
        f"<li style='margin-bottom: 6px;'><strong>Pass #{idx}</strong>:"
        f" {p['tier_name']} — Ref: <code"
        f" style='color:#7c3aed;'>{p['ticket_ref']}</code>"
        f" (₹{p['price']:,.2f})</li>"
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f8fafc; padding: 30px 20px; color: #0f172a;">
            <div style="max-width: 540px; margin: 0 auto; background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; padding: 28px;">
                ...
                <div style="text-align: center; margin-top: 20px;">
                    <a href="/dashboard" style="background: linear-gradient(135deg, #7c3aed, #db2777); color: #fff; text-decoration: none; padding: 10px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; display: inline-block;">Open My Dashboard</a>
                </div>
            </div>
        </div>
        """
  ...
  try:
    ...
    if not venue_name or not booking_date:
      return (
          jsonify({
              "status": "error",
              "message": "Venue name and date are required.",
          }),
          400,
      )
    ...
    if cursor.fetchone():
      ...
      return (
          jsonify({
              "status": "error",
              "message": "This venue is already booked/locked for this date.",
          }),
          409,
      )
    ...
    admin_alert_html = f"""
        <div style="font-family:sans-serif; padding:20px; color:#0f172a;">
            ...
            <p>Please log in to your <a href="/admin">Admin Panel</a> to allocate and approve this venue.</p>
        </div>
        """
    send_email_direct(
        ADMIN_EMAILS[0],
        f"Venue Request: {venue_name} ({booking_ref})",
        admin_alert_html,
    )
    ...
    return (
        jsonify({
            "status": "success",
            "message": (
                "Venue request submitted! Awaiting Admin Allocation before"
                " payment."
            ),
...
def admin_approve_venue():
  ...
  try:
    ...
    if not record:
      ...
      return (
          jsonify(
              {"status": "error", "message": "Booking request not found."}
          ),
          404,
      )
    ...
    cursor.execute(
        "UPDATE bookings SET status = 'APPROVED_AWAITING_PAYMENT' WHERE id = ?",
        (booking_id,),
    )
    conn.commit()
    conn.close()
    ...
    return (
        jsonify({
            "status": "success",
            "message": (
                "Venue allocated successfully! Approval email sent to"
                f" {user_email}."
            ),
        }),
        200,
...
def download_ticket_pdf(booking_id):
  ...
  header_data = [[
      Paragraph(
          "<b>evora</b><br/><font size=8 color='#64748b'>Official Verified"
          " Entry Pass & Tax Invoice</font>",
          title_style,
      ),
      Paragraph(
          f"<font color='#059669'><b>STATUS:"
          f" {b_status}</b></font><br/><b>Ref:</b> {b_ref}<br/><b>Date:</b>"
          f" {b_date}",
          meta_style,
      ),
  ]]
  ...
  line_data = [
      ...
      [
          Paragraph(f"<b>{b_title}</b>", meta_style),
          Paragraph(b_details, meta_style),
          f"INR {b_price:,.2f}",
      ],
  ]
  ...
  auth_data = [[
      rl_qr,
      Paragraph(
          "<b>Gate Security Instructions:</b><br/>Present this official QR code"
          f" at the event turnstile.<br/>Issued To: <b>{user_email}</b><br/>Payment"
          f" Ref ID: <b>{b_pay_id}</b><br/><font size=8"
          " color='#94a3b8'>Evora Live Event Operations Pvt Ltd • GSTIN:"
          " 27AABCE1234F1Z5</font>",
          meta_style,
      ),
  ]]
  ...
  return (
      jsonify({
          ...
          "stats": {
              ...
              "passes_count": (
                  sum(1 for b in bookings if b["type"] == "passes")
              ),
              "manpower_count": (
                  sum(1 for b in bookings if b["type"] == "manpower")
              ),
              "venues_count": (
                  sum(1 for b in bookings if b["type"] == "venues")
              ),
          },
      }),
      200,
...
def get_venue_booked_dates():
  ...
  if not venue_name:
    return (
        jsonify({"status": "error", "message": "Venue name is required"}),
        400,
    )
...
def admin_update_status():
  try:
    ...
    if notes:
      cursor.execute(
          "SELECT details FROM bookings WHERE id = ?", (booking_id,)
      )
      row = cursor.fetchone()
      cur_details = row[0] if row else ""
      if "Allocated:" not in cur_details:
        cursor.execute(
            "UPDATE bookings SET details = ? WHERE id = ?",
            (f"{cur_details} • Allocated: {notes}", booking_id),
        )
    ...
    return (
        jsonify({
            "status": "success",
            "message": f"Updated to '{new_status}' successfully.",
        }),
        200,
    )
  except Exception as e:
...
def admin_get_users_and_logs():
  ...
  cursor.execute(
      "SELECT id, fullname, email, phone, role, created_at FROM users ORDER BY"
      " id DESC"
  )
  users = [{
      "id": u[0],
  ...
  cursor.execute(
      "SELECT id, user_email, fullname, login_time, ip_address, status FROM"
      " login_logs ORDER BY id DESC LIMIT 50"
  )
  logs = [{
      "id": l[0],
      "email": l[1],
...
# ==========================================================================
# ADMIN TRANSACTION, USER DELETION & AUDIT PURGE APIS
# ==========================================================================
...
def admin_delete_user():
  ...
  if not target_email:
    return (
        jsonify({"status": "error", "message": "Target email is required."}),
        400,
    )
  ...
  try:
    ...
    return (
        jsonify({
            "status": "success",
            "message": f"Account {target_email} successfully removed.",
        }),
        200,
    )
  except Exception as e:
...
@app.route(
    "/api/admin/delete-booking/<int:booking_id>", methods=["DELETE", "POST"]
)
@admin_required
def admin_delete_single_booking(booking_id):
  ...
  try:
    ...
    cursor.execute(
        "SELECT booking_ref, title FROM bookings WHERE id = ?", (booking_id,)
    )
    record = cursor.fetchone()
    if not record:
      conn.close()
      return (
          jsonify({"status": "error", "message": "Booking not found."}),
          404,
      )
    ...
    return (
        jsonify({
            "status": "success",
            "message": (
                f"Booking {record[0]} ({record[1]}) removed from pipeline."
            ),
        }),
        200,
    )
...
def host_custom_event():
  ...
  try:
    ...
    if not title or not venue or not date:
      return (
          jsonify(
              {"status": "error", "message": "All event details are required."}
          ),
          400,
      )
    ...
    if image_file and allowed_file(image_file.filename):
      filename = (
          f"event_{int(time.time())}_{secure_filename(image_file.filename)}"
      )
